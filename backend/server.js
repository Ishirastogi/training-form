...
const pool = new Pool({
  ...
  ssl: {
    rejectUnauthorized: false, // Only for development — skip SSL cert validation
  },
});
...
app.get("/get-data", async (req, res) => {
  try {
    ...
    if (role === "admin") {
      ...
      query =
        "SELECT * FROM training WHERE location = 'Unit 1' ORDER BY id DESC";
    } else if (role === "user") {
      query =
        "SELECT * FROM training WHERE location = 'Unit 2' ORDER BY id DESC";
    } else if (role === "hr") {
      query =
        "SELECT * FROM training WHERE location = 'Unit 3' ORDER BY id DESC";
    } else {
